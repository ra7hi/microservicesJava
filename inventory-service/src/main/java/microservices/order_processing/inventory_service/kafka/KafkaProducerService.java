package microservices.order_processing.inventory_service.kafka;
...
public class KafkaProducerService {
    ...
    public void sendSagaEvent(SagaEvent sagaEvent) {
        ...
        future.whenComplete((result, ex) -> {
            if (ex == null) {
                log.info("Saga event sent successfully from inventory service: {}", sagaEvent.getSagaId());
            } else {
                log.error("Failed to send saga event from inventory service: {}, error: {}",
